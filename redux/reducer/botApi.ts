...
const dynamicBaseQuery: BaseQueryFn<String | FetchArgs, any, FetchBaseQueryError> = async (
   args: any,
   api,
   extraOptions
) => {
   ...
   const raw = fetchBaseQuery({
      baseUrl: `https://api-americas-instance1.eva.bot/eva-broker/org/${orgUUID}/env/${envUUID}/bot/${botUUID}/conversations`,
      prepareHeaders: async (headers, { getState }) => {
         const token = (getState() as RootState).app.token;

         if (!token) {
            const at = await fetch(
               'https://keycloak-americas-admin.eva.bot/auth/realms/NTTDATA-EMEAL/protocol/openid-connect/token',
               {
                  method: 'post',
                  headers: {
               ...
               .then((el) => el.json())
               .then((res) => {
                  api.dispatch(setToken(res.access_token));
                  return res;
               })
